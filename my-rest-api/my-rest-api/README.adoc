...
The project **monApiExtension** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/monApiExtension.git
cd monApiExtension/
./mwnw package
----

The build should produce a zip archive under the `target/` folder named `my-rest-api-1.0-SNAPSHOT.zip`
...
Take the built artifact `my-rest-api-1.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
